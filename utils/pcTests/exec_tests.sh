...
function execTest
{
    ...
    ../../scripts/fastbuffers_local.sh -o output -ser cdr -local -example $EPROSIMA_TARGET "$1/$1.idl"
    errorstatus=$?
    ...
    # Generates the file with RPCDDS script
    ../../scripts/fastbuffers_local.sh -o output -ser fastcdr -local -example $EPROSIMA_TARGET "$1/$1.idl"
    errorstatus=$?
    if [ $errorstatus != 0 ]; then return; fi
    # Compile the generated library
    make -C output -f makefile_$EPROSIMA_TARGET all
